...
git diff --exit-code -- apps/ios_probe/VoiceProbe.xcodeproj

swift format lint --recursive --strict apps/ios_probe
plutil -lint apps/ios_probe/config/*.plist >/dev/null

xcrun simctl bootstatus "$simulator_udid" -b
xcrun simctl privacy "$simulator_udid" grant microphone \
  com.longdevity.hardwarecontroller.voiceprobe
